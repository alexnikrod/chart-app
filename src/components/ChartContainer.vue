...
export default {
  ...
  methods: {
    async setChartData() {
      ...
      for (let i of chartData) {
        if (this.inputData[i].data) {
          ...
          const calculatedData = await this.getData({
            outsideDiameter,
            insideDiameter,
            yieldStress,
            scale,
    ...
    setSingleLoadsData() {
      const singleLoadsChartData = this.singleLoadsData
        ...
        .map((item) => {
          return {
            ...
            data: {
              data: [
                {
                  x: item.data ? item.data.pressure : null,
                  y: item.data ? item.data.force : null,
                },
              ],
            },
          };
      ...
      const chartData = this.chartData.filter(
        (item) => !item.id.includes("single-load")
      );
